...
class HiddenStateDiffusionModel(ModelMixin, ConfigMixin):
    ...
    一个改进版本的“条件扩散”模型示例：
      ...
      - 使用 (sample - draft) 的差值作为目标进行归一化，再预测差值残差，
        最后通过反归一化加回 draft 得到最终预测结果。
    """
    ...
    def __init__(
        ...
        diff_mean: float = -0.000527,    # (gt - draft) 的均值
        diff_std: float = 1.386598,      # (gt - draft) 的标准差
    ):
        super().__init__()
        ...
        # 先把 (normalized diff + time_embed) 投影到 hidden_dim
    ...
    def forward(
        ...
        **kwargs
        ...
        # 3. 计算 diff = sample - draft，并归一化
        diff = sample - draft  # [B, hidden_dim]
        normalized_diff = (diff - self.diff_mean) / (self.diff_std + 1e-8)

        # 4. 拼接归一化后的 diff 与时间步嵌入
        x = torch.cat([normalized_diff, t_emb], dim=-1)  # 形状 [B, hidden_dim + time_embed_dim]
        ...
        # 7. 输出层，得到归一化后的 residual（预测的差值 residual）
        normalized_residual = self.output_layer(x)

        # 8. 反归一化 residual，将其转换到原始尺度
        residual = normalized_residual * (self.diff_std + 1e-8) + self.diff_mean

        # 9. 最终预测结果：draft + residual
        pred = draft + residual
